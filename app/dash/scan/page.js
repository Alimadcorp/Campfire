...
export default function ScanPage() {
  ...
  const [transitModal, setTransitModal] = useState(null);
  ...
  useEffect(() => {
    ...
    try {
      scanCache.current = JSON.parse(
        localStorage.getItem("cf_scan_cache_2") || "{}",
      );
    } catch (e) {
      scanCache.current = {};
    }
  ...
  useEffect(() => {
    const id = setInterval(() => setNowMs(Date.now()), 7);
    ...
    fetch("/api/event/table")
      ...
      .then((data) => {
        const merged = (data.participants || []).map((p) => {
          const f =
            data.fillout?.find((x) => String(x.id) === String(p.id)) ||
            data.fillout?.find((x) => x.emailC === p.email);
          return {
            ...p,
            hasFillout: !!f,
            cnic: f?.cnic || "",
  ...
  const connect = useCallback(() => {
    ...
    const host =
      typeof window !== "undefined" && location.host.endsWith(":5500")
        ? "ws://localhost:8080"
        : "wss://ws.campfire.alimad.co";
    ...
    ws.onopen = () => {
      ...
      ws.send(
        JSON.stringify({
          ...
          password: pw,
          role: "listener",
          user: sessionRef.current,
        }),
      );
    };
    function onScan(d) {
      setScans((prev) =>
        [{ ...d, _isNew: true, _status: "idle" }, ...prev].slice(0, 200),
      );
    }
    function onManualScan(text) {
      const ids = text.split(/[\s,;]+/).filter(x => x.trim());
      ids.forEach(id => {
        const d = {
          "type": "scan",
          "data": id,
    ...
    window.onManualScan = onManualScan;
    ws.onmessage = (msg) => {
      try {
        ...
        if (d.type === "history") {
          ...
          setScans((prev) => {
            const existingKeys = new Set(
              prev.map((x) => `${x.time}|${x.data}`),
            );
            const filteredNew = newScans.filter(
              (s) => s.data && !existingKeys.has(`${s.time}|${s.data}`),
            );
        ...
        if (d.type === "scan_history") {
          setScanHistory(d.scans || []);
          setTimelineLoading(false);
        }
        if (d.type === "online_devices") {
          setDevices(d.devices || []);
          setDeviceMeta(d.metadataMap || {});
        }
  ...
  useEffect(() => {
    const pending = scans.filter(
      (s) =>
        s._status === "idle" &&
        s.data &&
        !processedScans.current.has(`${s.time}|${s.data}`),
    );
    ...
    pending.forEach((scan) => {
      ...
      if (cached) {
        const ns =
          cached.status === "already_scanned" ||
            cached.status === "checkin_completed"
            ? "already"
            : "valid";
        if (cached.status === "not_found") {
          setScans((prev) =>
            prev.map((s) =>
              s.time === scan.time && s.data === scan.data
                ? { ...s, _status: "invalid", _scanResult: cached }
                : s,
            ),
          );
        } else {
            ...
            prev.map((s) =>
              s.time === scan.time && s.data === scan.data
                ? {
                  ...
                  _status: ns,
                  _scanResult: cached,
                  _participantId: cached.participant?.id,
                }
                : s,
            ),
          );
      ...
      fetch(
        `/api/event/scan?day=${getDay()}&id=${encodeURIComponent(scan.data)}`,
      )
        ...
        .then((res) => {
          ...
          localStorage.setItem(
            "cf_scan_cache_2",
            JSON.stringify(scanCache.current),
          );
          ...
          if (res.status === "not_found") {
            setScans((prev) =>
              prev.map((s) =>
                s.time === scan.time && s.data === scan.data
                  ? { ...s, _status: "invalid", _scanResult: res }
                  : s,
              ),
            );
          } else {
              ...
              prev.map((s) =>
                s.time === scan.time && s.data === scan.data
                  ? {
                    ...
                    _status: ns,
                    _scanResult: res,
                    _participantId: res.participant?.id,
                  }
                  : s,
              ),
            );
  ...
  async function fetchParticipant(id) {
    ...
    if (existing) {
      setParticipants((prev) => ({ ...prev, [id]: existing }));
      setExpanded((prev) => ({ ...prev, [id]: true }));
      return;
    }
    try {
      const r = await fetch(`/api/event/participant?id=${id}`);
      if (r.ok) {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ws.send(
        JSON.stringify({
          ...
          id,
          text: text.trim(),
          author: sessionRef.current.name,
  ...
  function setColorTag(id, c) {
    addNote(id, `🎨 TAG: ${c}`);
  }
  function setGameEntry(id, g) {
    if (!g?.trim()) return;
    addNote(id, `🎮 GAME: ${g.trim()}`);
    setGameInputs((prev) => ({ ...prev, [id]: "" }));
  }
  function assignTeam(id, t) {
    if (!t?.trim()) return;
    addNote(id, `🏷️ TEAM ASSIGNED: ${t.trim()}`);
    setTeamInputs((prev) => ({ ...prev, [id]: "" }));
  }
  ...
  function logLeave(id, name, reason) {
    addNote(
      id,
      `🚪 LEFT VENUE at ${new Date().toLocaleTimeString()}${reason ? ` — Reason: ${reason}` : ""}`,
    );
  ...
  function logArrival(id, name, reason) {
    addNote(
      id,
      `🚪 ARRIVED BACK at ${new Date().toLocaleTimeString()}${reason ? ` — Note: ${reason}` : ""}`,
    );
    setTransitModal(null);
    setTransitNote("");
  }
  ...
  function doConnect() {
    if (!channel.trim() || !password.trim())
      return setError("Channel and password required");
    setError("");
    setConnecting(true);
  ...
  function rescanAll() {
    ...
    scanCache.current = {};
    localStorage.removeItem("cf_scan_cache_2");


    setScans((prev) => {
      const combined = [...scanHistory, ...prev];
      const seen = new Set();
      const result = [];
      combined.forEach(s => {
        ...
        seen.add(key);
        result.push({ ...s, _status: "idle", _isNew: true });
      });
  ...
  const scannedIds = useMemo(() => {
    ...
    scans.forEach((s) => {
      ...
      if (s._scanResult?.participant?.id)
        ids.add(String(s._scanResult.participant.id));
    });
      ...
      allParts
        .map((p) => {
          ...
          return {
            ...
            _inVenue: inVenue,
            _meta: meta,
            _lastNoteTime: lastTime,
          };
        })
        .filter((p) => matchSearch(p, leftSearch))
        .filter((p) => {
          if (filters.vol === true && !p.isVolunteer) return false;
          if (filters.vol === false && p.isVolunteer) return false;
          if (
            filters.org === true &&
            !ALLOWED_PII_VIEWERS.includes(String(p.id))
          )
            return false;
          if (
            filters.org === false &&
          ...
          if (filters.sin === true && !p.scannedDay2) return false;
          if (filters.sin === false && p.scannedDay2) return false;
          return true;
        })
        .filter((p) => {
  ...
  const sortedFiltered = useMemo(
    () =>
      [...leftFiltered].sort((a, b) => {
        ...
        if (sortBy === "signupDay2")
          return new Date(b.createdTime || 0) - new Date(a.createdTime || 0);
        if (sortBy === "cin")
          return (b.checkinCompleted ? 1 : 0) - (a.checkinCompleted ? 1 : 0);
        if (sortBy === "exp")
          return (Number(b.exp) || 0) - (Number(a.exp) || 0);
        if (sortBy === "rating")
          return (b._meta?.rating || 0) - (a._meta?.rating || 0);
        if (sortBy === "lastNote")
          return (b._lastNoteTime || 0) - (a._lastNoteTime || 0);
        return 0;
      }),
    [leftFiltered, sortBy],
  );
  ...
  const venueList = useMemo(
    () => sortedFiltered.filter((p) => p._inVenue),
    [sortedFiltered],
  );

  const stats = useMemo(() => {
    const day = getDay();
    const registered = allParts.filter((p) => !p.disabled).length;


    const checkedInToday = allParts.filter((p) => {
      if (p.disabled) return false;
      return day === 1 ? (p.scanned || p.pendingScanned) : (p.scannedDay2 || p.pendingScannedDay2);
    }).length;


    const inVenue = allParts.filter((p) => {
      if (p.disabled) return false;
      const hasScannedToday = day === 1 ? (p.scanned || p.pendingScanned) : (p.scannedDay2 || p.pendingScannedDay2);
      if (!hasScannedToday) return false;
    ...
    const vStaff = leftFiltered.filter(
      (p) => p._inVenue && p.isVolunteer,
    ).length;
    const vParts = inVenue - vStaff;

    return { total: registered, scanned: checkedInToday, inVenue, vStaff, vParts, out: checkedInToday - inVenue };
  }, [allParts, leftFiltered, notes]);

  const [devicesModalOpen, setDevicesModalOpen] = useState(false);

  useEffect(() => {
    setPage(0);
  }, [leftSearch, filters, customFilters, sortBy, leftMode]);
  ...
  const timelineEntries = useMemo(() => {
    ...
    Object.entries(notes).forEach(([pid, pNotes]) => {
      (pNotes || []).forEach((n) => {
        ...
        let type = "note";
        if (text.startsWith("🚪"))
          type = text.includes("ARRIVED") ? "arrival" : "departure";
        else if (text.startsWith("🏷️")) type = "team";
        else if (text.startsWith("🎮")) type = "game";
        events.push({
          time: typeof ts === "number" ? new Date(ts).toISOString() : ts,
          _type: type,
          _participantId: pid,
          _noteText: text,
          _noteAuthor: n.author,
          _noteAvatar: n.avatar,
        });
      });
    });
  ...
  const { modalP, modalNotes, modalMeta, modalVisible } = useMemo(() => {
    const p = modalId
      ? participants[modalId] || allParts.find((x) => x.id === modalId)
      : null;
    const mn = modalId ? notes[modalId] || [] : [];
    return {
      modalP: p,
      modalNotes: mn,
      modalMeta: parseMeta(mn),
      modalVisible: visibleNotes(mn),
    };
  }, [modalId, participants, allParts, notes]);

  const listData = useMemo(
    () => (leftMode === "venue" ? venueList : sortedFiltered),
    [leftMode, venueList, sortedFiltered],
  );
  const totalPages = Math.max(1, Math.ceil(listData.length / PAGE_SIZE));
  const safePage = Math.min(page, totalPages - 1);
  const pageData = useMemo(
    () => listData.slice(safePage * PAGE_SIZE, (safePage + 1) * PAGE_SIZE),
    [listData, safePage],
  );
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center font-mono">
        <div className="w-80 bg-[#080808] border border-[#141416] rounded-lg p-8 flex flex-col gap-5 text-left">
          <p className="text-[0.8rem] font-black tracking-[0.2em] text-white text-center">
            CAMPFIRE
          </p>
          <div className="flex flex-col gap-1">
            <label className="text-[0.6rem] font-extrabold tracking-[0.15em] uppercase text-[#444]">
              Channel
            </label>
            <input
              value={channel}
              ...
              className="bg-[#0c0c0e] border border-[#141416] rounded px-3 py-2 text-sm text-white outline-none focus:border-blue-500 font-mono"
    ...
    <div className="h-screen bg-black flex font-mono text-white overflow-hidden">
      ...
      {flashNotes.length > 0 && (
        <div className="fixed bottom-4 left-4 z-40 flex flex-col gap-2">
          {flashNotes.map((n) => (
            <div
              key={n.key}
              className="flex items-center gap-2 bg-[#020617]/95 border border-[#1f2937] rounded-full px-3 py-1 shadow-lg"
            >
              {n.avatar && (
                <img
                  src={n.avatar}
                  alt={n.author}
                  className="w-6 h-6 rounded-full object-cover border border-[#020617]"
                />
              )}
              <div className="flex flex-col text-left">
                <span className="text-[0.6rem] text-[#9ca3af] font-bold">
                  {n.author} → {n.participantName}
                </span>
                <span className="text-[0.6rem] text-[#e5e7eb] truncate max-w-[220px]">
                  {n.text}
                </span>
              </div>
            </div>
      ...
      {rightPanelOpen && (
        <ScanEngine
          ...
          devices={devices}
          deviceMeta={deviceMeta}
          setDevicesModalOpen={setDevicesModalOpen}
        />
      )}

      {/* Global UI Overlays */}
      <div
        className={`fixed bottom-4 left-1/2 -translate-x-1/2 transition-all duration-500 z-[999] ${rightPanelOpen ? "-translate-x-[240px]" : ""}`}
      >
